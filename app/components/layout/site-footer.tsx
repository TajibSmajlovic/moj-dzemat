...
function ContributionSection() {
  return (
    ...
    >
      <div className="border-secondary/30 border-t-4 bg-[linear-gradient(135deg,hsl(var(--card)),hsl(var(--accent)/0.45))] p-4 sm:p-5 lg:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <span className="bg-background/80 text-primary mb-3 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold tracking-wide uppercase shadow-xs">
              <Code2 className="h-3.5 w-3.5" aria-hidden="true" />
              Open Source
            </span>

            <h2
              id="footer-contribution-title"
              className="font-display text-foreground text-xl font-semibold text-balance sm:text-2xl"
            >
              Doprinesi aplikaciji
            </h2>
          ...
          <Button asChild size="lg" className="w-full rounded-full px-5 shadow-sm sm:w-auto">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {CONTRIBUTION_CONTACTS.map((contact) => (
              <a
                ...
                target="_blank"
                rel="noreferrer noopener"
                className="border-border bg-background/75 text-foreground hover:bg-accent hover:text-accent-foreground focus-visible:ring-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-full border px-4 text-sm font-medium shadow-xs transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
              >
                <ContactRound className="text-primary h-4 w-4" aria-hidden="true" />
                {contact.name}
