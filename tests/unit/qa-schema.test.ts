...
describe("Q&A schemas", () => {
  it("trims and accepts question length boundaries", () => {
    ...
    expect(QaSubmitSchema.safeParse({ question: "a".repeat(QA_QUESTION_MAX_LENGTH) }).success).toBe(
      true,
    );
  });
  ...
  it("rejects missing and out-of-range questions", () => {
    ...
    expect(
      QaSubmitSchema.safeParse({ question: "a".repeat(QA_QUESTION_MAX_LENGTH + 1) }).success,
    ).toBe(false);
  });
  ...
  it("trims and accepts answer length boundaries", () => {
    ...
    expect(QaAnswerSchema.safeParse({ answer: "a".repeat(QA_ANSWER_MAX_LENGTH) }).success).toBe(
      true,
    );
  });
  ...
  it("rejects missing and out-of-range answers", () => {
    ...
    expect(QaAnswerSchema.safeParse({ answer: "a".repeat(QA_ANSWER_MAX_LENGTH + 1) }).success).toBe(
      false,
    );
  });
});
